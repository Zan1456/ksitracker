      ...
      <PageTransition className="gap-4.5 overflow-y-auto px-5 pb-6 pt-5">
        <div>
          ...
          <p className="mb-3 text-[13px] leading-[1.55] text-text-muted">
            Legalább {settings.minRequired} feladatot teljesíts a {tasks.length}-ból — az eredmények
            felkerülnek a ranglistára, és a szint lezártnak számít.
          </p>
        ...
        <div className="mt-auto pt-1">
          {locked ? (
            ...
            <form action={startChallengeSessionAction.bind(null, levelId)}>
              <Button
                type="submit"
                size="lg"
                className="w-full"
                disabled={!limit.canStartNew || tasks.length === 0}
              >
              ...
              <p className="mt-2.25 text-center text-[11.5px] text-text-faint">
                {!limit.canStartNew || limitParam
                  ...
                  : "Indítás után ma már nem próbálkozhatsz újra"}
              </p>
            </form>
          )}
        </div>
